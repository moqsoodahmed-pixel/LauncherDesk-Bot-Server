...
// ── Per-process dedup store ──────────────────────────────────
// MSG91 fires the same webhook twice within ~1s. The per-phone
// lock (runExclusive) serialises calls from the same number but
// does NOT suppress identical events — both still run.
// This Map catches exact duplicates: same messageId (or same
// phone+text+minute-bucket for messages without a real ID) within
// a 90-second window.
const _recentEvents = new Map();
function _isDuplicate(parsed) {
  const { phone, messageId, text } = parsed;
  // Build a stable key: real msg ID is best, fallback to content+minute
  const minuteBucket = Math.floor(Date.now() / 60000);
  const key = messageId && !String(messageId).startsWith('local_')
    ? `${phone}_${messageId}`
    : `${phone}_${(text || '').trim().toLowerCase().slice(0, 40)}_${minuteBucket}`;

  if (_recentEvents.has(key)) {
    console.log(`[Webhook] Duplicate suppressed: ${key}`);
    return true;
  }
  _recentEvents.set(key, Date.now());
  // Clean up entries older than 90 seconds to prevent memory leak
  if (_recentEvents.size > 500) {
    const cutoff = Date.now() - 90000;
    for (const [k, t] of _recentEvents) {
      if (t < cutoff) _recentEvents.delete(k);
    }
  }
  return false;
}
...
app.post('/webhook/whatsapp', async (req, res) => {
  ...
  try {
    ...
    // Suppress MSG91 duplicate webhook fires
    if (_isDuplicate(parsed)) return;
