...
async function handleMenu(session, parsed) {
  ...
  if (!flowId) {
    ...
    // Re-show the menu. For stale taps we do it silently;
    // for genuine free text we add a brief notice — but still
    // only ONE message goes out (no fallback + menu double-send).
    await messages.sendWelcomeMenu(phone, session.state);
    return;
  }
